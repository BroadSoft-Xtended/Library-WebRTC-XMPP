...
function XMPP(debug, eventbus, dms, cookieconfig) {
  ...
  self.updateHasSubscriptions = function(subscriptions) {
    self.hasSubscriptions = subscriptions && Object.keys(subscriptions).length;
  }
  ...
  'contactSelected', 'subscriptions', 'hasSubscriptions', 'removeContactFailed', 'myContact'];

  self.bindings = {
    classes: {
      xmpp: ['visible', 'enableXMPP', 'connected', 'contactSelected', 'hasSubscriptions']
    },
    contactSelected: {
      xmpp: 'contact'
    },
