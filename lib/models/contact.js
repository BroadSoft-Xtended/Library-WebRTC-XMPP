module.exports = require('bdsft-sdk-model')(Contact);
...
function Contact(item, eventbus, client) {
	...
	self.props = ['id', 'full', 'name', 'messages', 'presence', 'subscription', 'classes', 'selected'];
	...
	self.init = function() {
		...
		self.full = item.jid && item.jid.full || item.full;
	...
	self.hasToSubscription = function(){
		return self.subscription === 'to' || self.subscription === 'both';
	};

	self.hasFromSubscription = function(){
		return self.subscription === 'from' || self.subscription === 'both';
	};
